...
import { fetchConversationEvidence, fetchCustomerChatEvidence, fetchDailyChatOrderSummary, fetchDailyOrderHistory, fetchExternalChatMessages, fetchLiveOrder, fetchLiveOrders, fetchOrdersForThread, fetchLiveProductMappings, fetchLiveThreads, fetchStockProducts, fetchStockWarnings, getLiveOrderStats, syncProductAliasToMaster, updateProductMapAlias, updateStockProduct } from "./supabase";
...
export const appRouter = router({
  ...
  orders: router({
    ...
    dailyOrderHistory: protectedProcedure.input(z.object({ date: z.string().regex(/^\d{4}-\d{2}-\d{2}$/), search: z.string().optional() })).query(({ input }) => fetchDailyOrderHistory(input.date, input.search)),
