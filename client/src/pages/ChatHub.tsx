...
import { Check, ChevronDown, CircleUserRound, Clipboard, Clock3, ImagePlus, MessageCircle, RefreshCw, Search, Send, Sparkles, Users, X } from "lucide-react";
...
function imageUrls(value: string | null | undefined) {
  if (!value) return [];
  try {
    const attachments = JSON.parse(value) as Array<{ url?: string }>;
    return attachments.map(item => item.url).filter((url): url is string => Boolean(url));
  } catch {
    return [];
  }
}

function isPageTimelineLine(line: string) {
  return /\[\s*เพจ\s*[:：]/i.test(line);
}

function isCustomerTimelineLine(line: string) {
  return /\[\s*ลูกค้า\s*[:：]/i.test(line);
}
...
export default function ChatHub() {
  ...
  const availablePages = useMemo(() => Array.from(new Set(threads.map(thread => thread.pageName).filter(page => page && page !== "ไม่ระบุเพจ"))).sort(), [threads]);
  const filteredThreads = useMemo(() => {
    const query = search.trim().toLowerCase();
    return threads.filter(thread => {
      const matchesPage = selectedPages.length === 0 || selectedPages.includes(thread.pageName);
      const matchesSearch = !query || [thread.customerName, thread.pageName, thread.preview, thread.latestOrderNumber].some(value => String(value ?? "").toLowerCase().includes(query));
      return matchesPage && matchesSearch;
    });
  }, [threads, search, selectedPages]);
  ...
  return <div className="min-h-[calc(100vh-2rem)] bg-[#09070d] text-white"><div className="mx-auto max-w-[1600px] space-y-5 p-2 sm:p-4 lg:p-6">
    ...
    {summaryPopup}<footer className="flex items-center gap-2 px-2 text-[11px] text-violet-100/30"><MessageCircle className="h-3.5 w-3.5" />n8n API → Supabase ทุก 1 นาที · ห้องแชทอ่านจากตารางกลาง · ทุกการตอบกลับบันทึก Audit Log</footer>
  </div></div>;
}
