...
// by occurred_at). Everything else below (bb_orders / chat_customer_evidence
...
async function fetchLinkedOrders(pageId: string, customerName: string) {
  ...
  const { data, error } = await supabase.from("bb_orders").select("*").eq("page_id", pageId).eq("customer_name", customerName);
  if (error) throw error;
